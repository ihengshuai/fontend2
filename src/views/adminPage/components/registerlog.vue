<template>
    <div class="registerlog">
        <Row style="padding:10px 0px 30px 0px;">
            ...
            <Col span="4">
                <Input prefix="logo-buffer" v-model="whatregisterlog" clearable @keyup.enter.native="queryloginlog()" placeholder="搜索日志(用户名优先)..." style="width: auto" />
            </Col>
            <Col span="4">
...
export default {
    ...
    methods:{
        ...
        handleCurrentChange(page) {
            ...
            // 设置默认分页数据
            this.allRegisterLog = table.filter((item, index) => {
        ...
        handleSizeChange(page_size) {
            ...
            this.allRegisterLog = this.allTableData.filter((item, index) => {
        ...
        // 获取所有注册日志
        getLoginLogs(){
            this.$axios.get("http://localhost:3001/api/admin/registerlog")
                    .then(res => {
                        this.allRegisterLog = res.data;
        ...
        // 查询某个人的注册日志
        queryloginlog(){
            if(this.whatregisterlog.trim().length){
                this.$axios.post("http://localhost:3001/api/admin/search/registerlog", {queryregisterlog:this.whatregisterlog})
                    .then(res => {
                        this.allRegisterLog = res.data;
                        this.allTableData = res.data;
                        this.setPaginations();
