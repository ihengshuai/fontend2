                        ...
                        <ul>
                            <li class="mobile-playlist-li" @click="playSelectedVideo(item,index)" v-for="(item, index) in allVideos" :key="item._id">
                                <div class="left-poster">
                                    <img :src="require('../../assets/vipcourse/css/' + item.poster)" alt="">
                                </div>
...
export default {
    ...
    methods:{
        ...
        getAllVideos(){
            // if(this.$route.query.seid){
                ...
                this.$axios.post("http://localhost:3001/api/vip/course/seid", createdVideo)
                    .then(res => {
                        this.playerOptions.sources[0].src = require("../../assets/vipcourse/css/" + res.data.src);
                        this.playerOptions.poster = require("../../assets/vipcourse/css/" + res.data.poster);
                        this.currentVideo = res.data;
                        this.currrentVideoIndex = parseInt(res.data.index);
                        this.showVideoPlayer = true;
                        this.$router.push({path:"/home/vip/course/css/play",query:{seid:res.data._id}})
                        setTimeout(() => {
        ...
        playSelectedVideo(info, index){
            ...
            this.playerOptions.sources[0].src = require("../../assets/vipcourse/css/" + info.src);
            this.playerOptions.poster = require("../../assets/vipcourse/css/" + info.poster);
