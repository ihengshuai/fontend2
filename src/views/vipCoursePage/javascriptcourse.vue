            ...
            <div class="left-container">
                ...
                <div class="video-container">
                    ...
                    <div class="title">
                        ...
                        <span class="content">JavaScript教程 <i>:</i><i>{{currentVideo.title}}</i></span>
                    </div>
                    <!-- 视频播放器 -->
                    ...
                    <div class="foot-comment">
                        ...
                        <div class="share" style="cursor:pointer;">
                            <social-sharing :url="'http://localhost:8080/home/vip/course/js/play?seid=' + currentVideo._id"
                                :title="'[前端教程 | by Mr_Wei], ' + currentVideo.title"
                                description="前端教程"
                        ...
                        <ul>
                            <li class="mobile-playlist-li" @click="playSelectedVideo(item,index)" v-for="(item, index) in allVideos" :key="item._id">
                                <div class="left-poster">
                                    <img :src="'http://localhost:3001/api/vip/course/js/poster?src=' + item.poster" alt="">
                                </div>
                        ...
                        <Row>
                            <Col span="2" class="comment-useravatar">
                                <Tooltip placement="top">
                                    <Avatar class="useravatar-message" icon="ios-person" :src="`http://localhost:3001/api/user/avatar?email=${currentUser.email}`" size="large" />
                                    <div slot="content">
                        ...
                        <div class="each-comment-box" v-if="allMessagesCount">
                            <Card class="comment-card" style="" v-for="(item, index) in allMessages" :key="index">
                                <p slot="title" style="height:25px;line-height:25px;">
                                    <!-- <span>{{1}}#</span> -->
                                    <Avatar :src="'http://localhost:3001/api/user/avatar?email=' + item.email" size="small"></Avatar>
                    ...
                    <div class="playlist">
                        <div id="playlist-detail" style="width:100%;" v-if="showsspecificvideo">
                            <div class="specilvideo" @click="playSelectedVideo(item,index)" v-for="(item, index) in allVideos" :key="item._id">
                                ...
                                第{{index + 1}}集 <i>JavaScript教程<i>{{item.title}}</i></i>
                            </div>
                        </div>
...
export default {
    ...
    methods:{
            ...
            this.$axios.get("http://localhost:3001/api/vip/course/js/all")
                    .then(res => {
                        if(res.data.length > 0){
                            ...
                            this.playerOptions.sources[0].src = "http://localhost:3001/api/vip/course/js/video?src=" + res.data[0].src;
                            this.playerOptions.poster = "http://localhost:3001/api/vip/course/js/poster?src=" + res.data[0].poster
        ...
        playSelectedVideo(info, index){
            ...
            this.playerOptions.sources[0].src = "http://localhost:3001/api/vip/course/js/video?src=" + info.src;
            this.playerOptions.poster = "http://localhost:3001/api/vip/course/js/poster?src=" + info.poster
            this.currrentVideoIndex = index;
            this.$router.push({path:"/home/vip/course/javascript/play",query:{seid:info._id}})
        ...
        selectVideoIndex(){
            ...
            this.$router.push({path:"/home/vip/course/javascript/play",query:{seid:this.currentVideo._id}})
            ...
            if(currentMessage.length >= 6 && currentMessage.length <= 85){
                ...
                setTimeout(() => {
                    if(!flag){
                        ...
                        this.$axios.post("http://localhost:3001/api/vip/course/js/message/add", messageInfo)
                                .then(res => {
                                    wsmLoading.endLoading();
        ...
        delSelfMessage(info){
            this.$Modal.confirm({
                ...
                cancelText:"取消",
                onOk:()=>{
                    this.$axios.post("http://localhost:3001/api/vip/course/js/message/del", {
                        _id : info._id
                    }).then(res => {
                        this.getAllMessages();
        ...
        // 获取所有的留言
        getAllMessages(){
            this.$axios.get("http://localhost:3001/api/vip/course/js/message/all")
                    .then(res => {
        ...
        $route(from,to){
            if(from.path == to.path){
                ...
                if(createdVideo._id){
                    ...
                    .then(res => {
                        if(res.data.length){
                            this.playerOptions.sources[0].src = "http://localhost:3001/api/vip/course/js/video?src=" + res.data.src;
                            this.playerOptions.poster = "http://localhost:3001/api/vip/course/js/poster?src=" + res.data.poster;
        ...
        .left-container{
            ...
            .videooutline{
                ...
                .video-poster{
                    width: 150px;
                    height: 100px;
                    background-image: url("../../assets/imgs/vip_javascriptcourse.png");
                    background-size: 100%;
                }
                .content{
                    flex:1;
