                        ...
                        <ul>
                            <li class="mobile-playlist-li" @click="playSelectedVideo(item,index)" v-for="(item, index) in allVideos" :key="item._id">
                                <div class="left-poster">
                                    <img :src="require('../../assets/vipcourse/javascript/' + item.poster)" alt="">
                                </div>
...
export default {
    ...
    methods:{
        ...
        getAllVideos(){
            // if(this.$route.query.seid){
                ...
                this.$axios.post("http://localhost:3001/api/vip/course/seid", createdVideo)
                    .then(res => {
                        this.playerOptions.sources[0].src = require("../../assets/vipcourse/javascript/" + res.data.src);
                        this.playerOptions.poster = require("../../assets/vipcourse/javascript/" + res.data.poster);
                        this.currentVideo = res.data;
                        this.currrentVideoIndex = parseInt(res.data.index);
                        this.showVideoPlayer = true;
                        this.$router.push({path:"/home/vip/course/javascript/play",query:{seid:res.data._id}})
                        setTimeout(() => {
        ...
        playSelectedVideo(info, index){
            ...
            this.playerOptions.sources[0].src = require("../../assets/vipcourse/javascript/" + info.src);
            this.playerOptions.poster = require("../../assets/vipcourse/javascript/" + info.poster);
