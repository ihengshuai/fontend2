<template>
    <div class="javascript" id="javascript">
        <div class="outer-box">
            <div class="left-bar">
                ...
                <Menu theme="light" style="z-index:10;" :active-name="activeName" accordion class="menu">
                    <MenuItem :to="'/home/javascript?title=' + item.title" :name="parseInt(index)" v-for="(item,index) in articlenav" :key="item.title"><Icon type="logo-javascript" />{{item.title}}</MenuItem>
                </Menu>
            </div>
        ...
        <!-- 移动端导航抽屉 -->
        <Drawer style="overflow-y:scroll;" title="JavaScript导航" v-model="mobileDrawer">
            <Menu theme="light">
                <MenuItem @click.native="shutDrawer" :to="'/home/javascript?title=' + item.title" :name="parseInt(index)" v-for="(item,index) in articlenav" :key="item.title"><Icon type="ios-hammer" />{{item.title}}</MenuItem>
            </Menu>
        </Drawer>
    </div>
</template>
...
export default {
    ...
    methods:{
        ...
        getArticle(){
            ...
            this.$axios.post("http://localhost:3001/api/article/type/javascript", {title})
                    .then(res => {
                            this.article = res.data.content;
                    }).catch(error => this.$router.push({path:"/home/javascript", query:{title:'CSS简介'}}))
        },
        // 判断序号
        getIndex(title){
            var Index = "";
        ...
        prev(){
            ...
            if(index == 0){
                ...
                this.$router.push({path:"/home/javascript",query:{title:title}})
            }
        },
        // 下一篇
        next(){
