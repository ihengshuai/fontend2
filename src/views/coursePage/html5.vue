<template>
    <div class="html5" id="html5">
        <div class="outer-box">
            <div class="left-bar">
                ...
                <Menu theme="light" style="z-index:10;" :active-name="activeName" accordion class="menu">
                    <MenuItem :to="'/home/html5?title=' + item.title" :name="parseInt(index)" v-for="(item,index) in articlenav" :key="item.title"><Icon type="logo-html5" />{{item.title}}</MenuItem>
                </Menu>
            </div>
            <div class="right-content" id="right-content">
                
                <mavon-editor id="markdownshow" :subfield="false" :toolbarsFlag="false" :value="article" v-model="article" :defaultOpen="preview">
        ...
        <!-- 移动端导航抽屉 -->
        <Drawer style="overflow-y:scroll;" title="HTML5导航" v-model="mobileDrawer">
            <Menu theme="light">
                <MenuItem @click.native="shutDrawer" :to="'/home/html5?title=' + item.title" :name="parseInt(index)" v-for="(item,index) in articlenav" :key="item.title"><Icon type="ios-hammer" />{{item.title}}</MenuItem>
            </Menu>
        </Drawer>
    </div>
</template>
    ...
    methods:{
        ...
        getArticle(){
            ...
            this.$axios.post("http://localhost:3001/api/article/type/html", {title})
                    ...
                    }).catch(error => this.$router.push({path:"/home/html5", query:{title:'HTML简介'}}))
        },
        // 判断序号
        getIndex(title){
            var Index = "";
        ...
        prev(){
            ...
            if(index == 0){
                ...
                this.$router.push({path:"/home/html5",query:{title:title}})
            }
        },
        // 下一篇
        next(){
